...
class VoiceFeedback:
    ...
    def _set_voice_gender(self, gender='female'):
        ...
        # If no exact match, try to find by index (usually index 1 is female on many systems)
        if selected_voice is None and len(voices) > 1:
            if target_gender == 'female':
                selected_voice = voices[1]  # Often female voice
            else:
                selected_voice = voices[0]  # Often male voice
